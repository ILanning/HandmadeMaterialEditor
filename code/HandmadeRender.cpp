...
void RenderScene(game_input *input, GameState *state)
{
	...
	if (rotateDirection.MagnitudeSquared() != 0)
	{
		...
		state->globals.Camera.Rotate(Quaternion::CreateFromAxisAngle(rotateDirection, rotateSpeed));
	}
	state->globals.Camera.rotation = state->globals.Camera.rotation.Normalize();
	...
	state->globals.arrow->Position += moveDirection;
